...
const ValidationRuleList = ({
  ...
  useEffect(() => {
    const fetchRulesWithService = async () => {
      ...
      try {
        // Add a getRules method to your validation service
        // This is a placeholder until you implement it in your service
        const rules = await fetchValidationRules(tableName, connectionId);
    ...
    fetchRulesWithService();
  }, [tableName, connectionId, validations.length, localValidations.length, loadingState, onUpdate]);

  // Helper function to fetch validation rules
  const fetchValidationRules = async (tableName, connectionId) => {
    // This is a temporary direct API implementation until you add this to your validationService
    try {
      console.log(`Direct fetch for validation rules for ${tableName}`);

      // Try a different approach - using the regular API but with special handling
      const response = await validationsAPI.getRules(
        tableName,
        {
          connectionId,
          forceFresh: true,
          timeout: 60000 // Extend timeout to ensure it doesn't time out
        }
      );
      ...
      // As a fallback, try another approach if the enhanced API fails
      try {
        ...
        const session = await import('../../../api/supabase').then(m => m.getSession());
        ...
        const backupResponse = await fetch(
          `${apiBaseUrl}/validations?table=${tableName}&connection_id=${connectionId}`,
          {
            headers: {
        ...
        // Extract rules from the backup response
        let backupRules = [];
        if (Array.isArray(backupData)) {
          backupRules = backupData;
        } else if (backupData?.rules && Array.isArray(backupData.rules)) {
          backupRules = backupData.rules;
        } else if (backupData?.data?.rules && Array.isArray(backupData.data.rules)) {
          backupRules = backupData.data.rules;
        }
